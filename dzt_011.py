...
mygpr = gp.gprpyProfile()
mygpr.importdata('/home/renata/ml-gpr/radargrama/FILE__011 P_1.DZT')
mygpr.setZeroTime(1.12)
...
inicio = x_min
fim = x_max
largura_janela = 0.8          # X-range
passo = 0.8                   # deslocamento
...
while x + largura_janela <= fim:
    # xrng são informações utilizadas para gerar o pdf na função 'mygpr.printProfile( )'
    ...
    # no parâmetro yrng: manter as informações
    # no parâmetro xrng: xrng = xrng
    # exemplo: mygpr.printProfile(nome_arquivo, color='', contrast=, yrng=yrng, xrng=xrng, dpi=600)
    mygpr.printProfile(nome_arquivo, color='gray', contrast=8.2, yrng=[0,8], xrng=xrng, dpi=600)
    ...
    pix.save(f"/home/renata/ml-gpr/imagens/dzt_011/frame_{i:02d}.jpg")
